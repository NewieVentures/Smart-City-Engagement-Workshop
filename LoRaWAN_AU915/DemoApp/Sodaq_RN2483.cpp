...
bool Sodaq_RN2483::initOTA(SerialType& stream, const uint8_t devEUI[8], const uint8_t appEUI[8], const uint8_t appKey[16], bool adr)
{
    ...
    return resetDevice() &&
//Optionally avoid override the dev's EUI by HR170521
        (!devEUI || setMacParam(STR_DEV_EUI, devEUI, 8)) &&
...
void Sodaq_RN2483::wakeUp()
{
    ...
    this->loraStream->flush();
    //Let any junk that appears after waking enter the loraStream before flushing. Added by HR170521
    delay(100); //In practice this is probably nothing, unless there's some old stuff left over from a previous command, eg. sleep()
    while(this->loraStream->available() > 0)
      this->loraStream->read(); //clear all incoming. Note flush() now only clears outgoing.
}
...
{
    debugPrint("[expectString] expecting ("); debugPrint(str); debugPrint(") "); //parentheses added by HR170521
    ...
    while (millis() < start + timeout) {
        ...
        if (readLn() > 0) {
            debugPrint(" got ("); debugPrint(this->inputBuffer); debugPrint(")"); //got and spacing added by HR170521
            ...
            debugPrintLn(" not a match."); //added by HR170521
            return false;
        }
    }
...
bool Sodaq_RN2483::resetDevice()
{
    ...
    else //added by HR170521
      debugPrintLn("Unexpected response while resetting device.");
...
{
    debugPrint("[setMacParam] "); debugPrint(paramName); debugPrint("= [");
    for (uint16_t i = 0; i < size; ++i) { //debug print of paramValue added by HR170521
        debugPrint(static_cast<char>(NIBBLE_TO_HEX_CHAR(HIGH_NIBBLE(paramValue[i]))));
        debugPrint(static_cast<char>(NIBBLE_TO_HEX_CHAR(LOW_NIBBLE(paramValue[i]))));
    }
    debugPrintLn("]");
